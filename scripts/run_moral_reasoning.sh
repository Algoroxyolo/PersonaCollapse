#!/usr/bin/env bash
#
# Unified moral-reasoning experiment runner.
#
...
#        bash scripts/run_moral_reasoning.sh <provider> <model>
#
#   2. Actually execute the experiment:
#        bash scripts/run_moral_reasoning.sh <provider> <model> --run
#
# Examples
# --------
...
#   bash scripts/run_moral_reasoning.sh openai gpt-5-mini --run
#   bash scripts/run_moral_reasoning.sh bedrock claude-haiku-4.5 --run
#
...
# -- Argument parsing -------------------------------------------------------
PROVIDER="${1:?Usage: bash scripts/run_moral_reasoning.sh <provider> <model> [--run]}"
MODEL="${2:?Usage: bash scripts/run_moral_reasoning.sh <provider> <model> [--run]}"
RUN_FLAG="${3:-}"
...
SCENARIOS_FILE="data/claude-3-5-sonnet_AB_0_with_logprobs.jsonl"
...
echo "========================================================"
echo "  Budget Estimation -- Moral Reasoning"
...
ESTIMATE_ARGS=(
    ...
    --model "${MODEL}"
    --provider "${PROVIDER}"
    --save-report "budget_moral_reasoning_${MODEL}.txt"
    --flagged-personas-file "${FLAGGED_FILE}"
)

if [ -f "${ROOT_DIR}/${SCENARIOS_FILE}" ]; then
    ESTIMATE_ARGS+=(--scenarios-file "${SCENARIOS_FILE}")
else
    echo "Note: ${SCENARIOS_FILE} not found locally."
    ESTIMATE_ARGS+=(--n-scenarios 50)
fi

python "${ROOT_DIR}/experiments/estimate_budget.py" "${ESTIMATE_ARGS[@]}"

if [ "$RUN_FLAG" != "--run" ]; then
    echo ""
    echo "To proceed, re-run with --run:"
    echo "  bash scripts/run_moral_reasoning.sh ${PROVIDER} ${MODEL} --run"
    exit 0
fi
...
DB_FILE="results_moral_${MODEL}.db"

python "${ROOT_DIR}/experiments/moral_reasoning.py" \
    --provider "${PROVIDER}" \
    --model "${MODEL}" \
    --api-key "${API_KEY}" \
    "${BASE_URL_ARGS[@]}" \
    ...
    --use-db 1 \
    --db-path "${DB_FILE}" \
    --output-file "results_moral_${MODEL}.json" \
    --flagged-personas-file "${FLAGGED_FILE}" \
